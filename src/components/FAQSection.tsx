...
export default function FAQSection({ faqRef }: FAQSectionProps) {
  ...
  return (
    <section 
      ref={faqRef}
      id="faq" 
      className="py-16 md:py-24 bg-slate-50/50 border-t border-slate-100"
    >
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-4 max-w-3xl mx-auto">
          {FAQS.map((faq, index) => (
            <div 
              key={index} 
              className="bg-white border border-slate-200/70 hover:border-indigo-100/60 rounded-2xl overflow-hidden transition-all duration-300 shadow-sm"
            >
              <button 
                id={`faq-btn-${index}`}
